...
if(window.location.pathname == '/index.html'){document.addEventListener('DOMContentLoaded', function() {//document (the page), add listener for an event (in this case its DOM (aka the document) kinda), which then calls to any function following it
    let num = parseInt(Math.random() * 30 + 1);                                                         //pulls an int from the 1/30 chance thing because it always outputs floats
...
//event listener and auto loadscripts for main menu
if(window.location.pathname == '/g/' || '/g/index.html'){document.addEventListener('DOMContentLoaded', function() {
  //add read cookie for settings later
  printIt(1, '/js/json/contents.json'); //calls the printIt function with 1 as the mode so it does default
});
}
